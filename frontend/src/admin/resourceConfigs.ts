...
export const RESOURCE_CONFIGS: Record<string, ResourceConfig> = {
  ...
  menus: {
    ...
    fields: [
      ...
      { key: 'url', label: 'URL (contoh /artikel)' },
      { key: 'location', label: 'Lokasi: header / footer' },
      { key: 'parent_id', label: 'Parent ID (kosong = top level)', type: 'number' },
      { key: 'sort_order', label: 'Urutan', type: 'number' },
  ...
  services: {
    ...
    fields: [
      ...
      { key: 'icon', label: 'Icon (file-text, calendar, ...)' },
      { key: 'color', label: 'Warna #hex' },
  ...
  ekstrakurikuler: {
    ...
    fields: [
      ...
      { key: 'icon', label: 'Icon (Users, Flag, Trophy, ...)' },
