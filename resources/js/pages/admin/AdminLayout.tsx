...
function SidebarChrome({
  ...
  return (
    <>
      ...
      <div className="border-t border-line dark:border-slate-800 bg-peach-soft/40 dark:bg-slate-900/60 p-2.5">
        <div className="mb-2 flex items-center gap-2.5 rounded-[12px] border border-line dark:border-slate-800 bg-white dark:bg-slate-800/80 px-2.5 py-2">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand text-[11px] font-bold text-white">
            {userInitials}
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-semibold text-ink dark:text-white">{userName}</p>
            <p className="truncate text-[11px] text-subtle dark:text-slate-400">{userEmail}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-1">
          <a
            ...
            rel="noreferrer"
            onClick={onNavigate}
            className="inline-flex items-center justify-center gap-1.5 rounded-[10px] px-2 py-2 text-xs font-semibold text-body dark:text-slate-300 transition hover:bg-white dark:hover:bg-slate-800 active:scale-[0.98]"
          >
...
export function AdminLayout({ children }: { children?: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-page lg:grid lg:grid-cols-[248px_1fr]">
      ...
      <BrandIcons />
      <motion.aside
        className="sticky top-0 hidden h-screen flex-col border-r border-line dark:border-slate-800 bg-white dark:bg-slate-900 lg:flex"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            />
            <motion.aside
              className="absolute inset-y-0 left-0 flex w-[min(100%,280px)] flex-col bg-white dark:bg-slate-900 shadow-xl"
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-line dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 px-4 py-3 shadow-[var(--shadow-header)] backdrop-blur md:px-6">
